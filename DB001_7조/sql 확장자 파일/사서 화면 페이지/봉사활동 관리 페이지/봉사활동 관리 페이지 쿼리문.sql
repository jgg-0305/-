...
WHERE
    ...
    AND (
        T1.req_title LIKE CONCAT('%', '데이터 사이언스 입문', '%') 
        OR T1.req_auth LIKE CONCAT('%', '김진, 최정아', '%')
...
SET
    req_stat = '반려',
    req_proc_memo = '예산 초과' -- [수정됨] 예: '학습 무관 도서', '예산 초과'
WHERE
    req_id = 1; -- [변수] 신청 번호
